...
figure()
h1 = plot(x_vals, y_vals, "r", "LineWidth", 2)
axis([-2, 2, -2, 2])
hold on
...
axis equal;
axis square;
h2 = plot(py_approx - 0.2, px_approx - 1.02, 'b--', "LineWidth", 2);
xlabel('X Position (m)');
ylabel('Y Position (m)');
...
for i = 1:length(t_vals_tan)
    ...
    T_x_val = double(subs(diff(x), t, t_vals_tan(i)));
    T_y_val = double(subs(diff(y), t, t_vals_tan(i)));

    T_norm = sqrt(T_x_val^2 + T_y_val^2);
    T_x_val = T_x_val / T_norm;
    T_y_val = T_y_val / T_norm;

    h3 = plot([x_pos, x_pos + T_x_val * scale], [y_pos, y_pos + T_y_val * scale], 'g-');

    N_x_val = -T_y_val; 
    N_y_val = T_x_val;

    N_x_val = N_x_val / sqrt(N_x_val^2 + N_y_val^2);
    N_y_val = N_y_val / sqrt(N_x_val^2 + N_y_val^2);

    h4 = plot([x_pos, x_pos + N_x_val * scale], [y_pos, y_pos + N_y_val * scale], 'c-');
end

t_vals_tan = [0, 5, 10, 15, 20, 25, 30]'; 
tan_indices = arrayfun(@(t) find(abs(tlist - t) == min(abs(tlist - t)), 1), t_vals_tan);

for i = 1:length(tan_indices)
    idx = tan_indices(i);
    
    x_pos = py_approx(idx) - 0.2;  
    y_pos = px_approx(idx) - 1.02;  
    
    vx_val = vx_approx(idx);
    vy_val = vy_approx(idx);
    
    norm = sqrt(vx_val^2 + vy_val^2);
    vx_tangent = (vx_val / norm) * scale;
    vy_tangent = (vy_val / norm) * scale;

    nx_val = vy_val / norm;  
    ny_val = -vx_val / norm;  

    nx_vector = nx_val * scale;
    ny_vector = ny_val * scale;

    h5 = plot([x_pos, x_pos + nx_vector], [y_pos, y_pos + ny_vector], 'g--');

    h6 = plot([x_pos, x_pos + vx_tangent], [y_pos, y_pos + vy_tangent], 'c--');
end

legend([h1, h2, h3, h4, h5, h6], "Expected", "Actual", "Expected Tangent", "Expected Normal", "Actual Tangent", "Actual Normal", "Location", "northeastoutside")
title("Parametric Curve")
xlabel("X Position (meters)")
ylabel("Y Position (meters)")